# 2019-nCov dataset of Nishiura et al. (2020).
...
# Load data
data <- read.table("data2019nCoV.txt")
x <- data[,c(2,3)]
...
pdf(file = "Fig3.pdf", width = 24, height = 8.5)
...
axis(1, at = seq(2,n,by=2), cex.axis = 2)
...
plot(sfine, sapply(sfine, SIfit$Fhat), type = "l", col = "red",
...
for(b in 1:B){
  lines(sfine, sboot_cdf[b,], type = "l", col = "#F0C488")
}
lines(sfine, sapply(sfine, SIfit$Fhat), type = "l", col = "red",
      lwd = 3)
...
for(j in 1:length(up_95)){
  ...
  lines(x = SIfit$estim$Estim[3:7], perc, type = "p", pch = 16,
        col = "red")
  text(x = up_95[j]+0.3, y = perc[j]-0.05, perctxt[j], offset = 1, cex = 1.5)
}
text(x = 1.15, y = 0.9, paste0("Mean SI (95% CI): ",
                               round(SIfit$estim$Estim[1],1)," (",
                               round(SIfit$estim$CI95p_l[1],1),
                               "-",
                               round(SIfit$estim$CI95p_r[1],1),")"), cex = 1.5)
text(x = 1, y = 0.8, paste0("SD SI (95% CI): ",
