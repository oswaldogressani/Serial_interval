...
#                           Scenario S7 (Measles)                             #
#------------------------------------------------------------------------------#
#   -->  Gamma SI with mean=9.9 and sd=2.4 from Vink et al. (2014)             #
#   -->  Sample size n = 100                                                   #
#   -->  Average coarseness of 2 days                                          #
#------------------------------------------------------------------------------#
...
library("EpiDelays")
library("xtable")
source("simSI_Gamma.R")

#---- Tune scenario setting here
S <- 1000
set.seed(1303)
n <- 100
meanSI  <-  9.9   # Vink et al. (2014)
sdSI    <-  2.4   # Vink et al. (2014)
Gshape <- (meanSI / sdSI)^2
Grate  <- meanSI / (sdSI^2)
qtrue   <- qgamma(p = c(0.05,0.25,0.50,0.75,0.95), shape = Gshape, rate = Grate)
nboot <- 2000
#-------------------------------
...
for(s in 1:S){
  simdata <- simSI_Gamma(muS = meanSI, sdS = sdSI, n = n)
...
print(xtable(perfnonparam, digits = 3, label = "Scenario-S7-Measles-n100"), 
      file="Scenario-S7-Measles-n100.txt")
